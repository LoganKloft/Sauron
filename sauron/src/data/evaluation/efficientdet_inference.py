...
# load config
f = open("./efficientdet_inference_config.json")
config = json.load(f)
f.close()
...
# load out file
conf = config["conf"]
f = open(f"efficientdet_inference_{int(conf * 100)}.txt", "w")

# load model
net = cv2.dnn_DetectionModel(
    "../../python/frozen_efficientdet_d0.pb", "../../python/efficientdet_d0.pbtxt"
)
